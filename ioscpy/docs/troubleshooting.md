...
`ioscpyctl status`, which reports the tweak state. Injected touches no longer
need a physical tap after respring: the tweak reads the digitizer sender id from
HID/IORegistry on load.
